...
import com.soulyaroslav.mvpbasestructure.navigation.factory.ActivityFactory;
import com.soulyaroslav.mvpbasestructure.navigation.factory.FragmentFactory;
...
//            tran.setCustomAnimations(R.anim.right_to_left_in, R.anim.right_to_left_out, R.anim.left_to_right_in, R.anim.left_to_right_out);
    ...
    private void switchActivity(ActivityPreference preferences) {
        ...
        if(preferences.isTransitionAnimation()) {
            List<Pair<View, String>> pairs = preferences.getTransitionViews();
            Pair[] transitionViews = new Pair[pairs.size()];
            for(int i = 0; i < pairs.size(); i++) {
                Pair<View, String> pair = pairs.get(i);
                transitionViews[i] = pair;
            }
            ActivityOptionsCompat options = makeSceneTransitionAnimation(activity, transitionViews);
